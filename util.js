 ...
 */
async function startCategoryQuizFromButtonWithFile(fileNames, questionCount, isIndex=false) {
    if (!fileNames) {
        alert('找不到檔案名稱！');
        return;
    }
	...
	await loadQuizFiles(JSON.stringify(filesArray), questionCount || '', isIndex);
}
...
async function loadQuizFiles(filesJson, questionCount, isIndex=false)
{
	if (!filesJson) {
		return;
	}
	...
	try {
		...
		utilGotoQuiz(finalQuestions, groupNames, 0, true, isIndex);
